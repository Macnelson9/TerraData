...
const displayCountryDetails = function () {
  ...
  const countryData2 = JSON.parse(sessionStorage.getItem('searchedCountry'));

  // Guard clause for country data
  if (!countryData && !countryData2) {
  ...
  } else if (countryData2) {
    const html = `
      <img src="${
        countryData2.flags.png
      }" alt="Country Flag" class="country-flag" />
      <div class="country-info2">
        <h2 class="country-name2">${countryData2.name.common}</h2>
        <p><strong>Native Name:</strong> <span>${
          countryData2.name.official
        }</span></p>
        <p><strong>Population:</strong> <span>${formatPopulation(
          countryData2.population
        )}</span></p>
        <p><strong>Region:</strong> <span>${countryData2.region}</span></p>
        <p><strong>Sub Region:</strong> <span>${
          countryData2.subregion || 'N/A'
        }</span></p>
        <p><strong>Capital:</strong> <span>${
          countryData2.capital || 'N/A'
        }</span></p>
        <p class="top-level-domain">
          <strong>Top Level Domain:</strong> <span>${
            countryData2.tld?.[0] || 'N/A'
          }</span>
        </p>
        <p><strong>Currencies:</strong> <span>${
          Object.values(countryData2.currencies)[0].name
        }</span></p>
        <p><strong>Languages:</strong> <span>${Object.values(
